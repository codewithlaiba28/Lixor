    ...
    <section className="relative px-6 max-w-[1200px] mx-auto mt-4 pb-12 z-10 flex flex-col items-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16 w-full">
        {filteredItems.map((item) => (
          ...
          >
            <div className="w-[210px] h-[210px] max-w-full aspect-square mb-6 transition-transform duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:scale-110 group-hover:rotate-[15deg] rounded-full overflow-hidden">
              <img
                src={item.image}
                alt={item.name}
            ...
            <div className="flex items-center gap-1.5 mb-2">
              <h3 className="font-serif font-medium text-[#1A1A1A] text-[18px] group-hover:text-[#FF5C00] transition-colors duration-300">
                {item.name}
              </h3>
              {item.logo && (
                <img
            ...
            <span className="text-[#999999] text-[14px] font-sans tracking-wide uppercase">
              {item.price}
            </span>
          </Link>
        ))}
